(ns demo.page.layout
  (:require
   [layout.flexlayout.core :refer [add-node  flexlayout-with-header]]
...
(defn header []
  [:div {:dir "ltr"
   ...
   [:button
    {:on-click #(add-node {:component "panel"
   ...
   [:button
    {:on-click #(add-node {:component "data"
                           :icon "/r/images/article.svg",
                           :name "Grid-added"})
     :style {:border-radius "5px"
             :border "1px solid lightgray"}}
   ...
   [:button
    {:on-click #(add-node {:component "algo"
                           :icon "/r/images/article.svg",
                           :name "algo1"
                           :id "algo1"
   ...
   [:button
    {:on-click #(add-node {:component "saying"
                           :name "saying"
                           :id "saying1"
                           :options {:id 0}
                           :edit [{:type :select
...
(defn page [_]
  [flexlayout-with-header header
   {:layout-json (clj->js model)
    :data {"aggrid" {:fun 'demo.service.aggrid/stock-csv}
