...
// Generate a dataset of 2000 points for the sine wave
const sineWaveData = [];
for (let i = 0; i < 2000; i++) {
    const amplitude = 30; // Height of the wave
    const frequency = 0.02; // Frequency of the wave
    sineWaveData.push(Math.sin(i * frequency) * amplitude);
}

// Config
let visibleBars = 200; // Show 200 candles and sine points at a time
let scrollIndex = 0; // Start at the beginning
const candleWidth = 10; // Fixed width for each candle
let maxScrollIndex = Math.min(candles.length, sineWaveData.length) - visibleBars;
...
function calculateVisibleBars() {
    ...
    maxScrollIndex = Math.min(candles.length, sineWaveData.length) - visibleBars;
...
function drawSineWave() {
    ...
    // Get visible sine wave points
    const visibleSineWave = sineWaveData.slice(scrollIndex, scrollIndex + visibleBars);

    // Map sine wave points to canvas x positions
    visibleSineWave.forEach((y, i) => {
        const x = i * candleSpacing + candleSpacing / 2;
        sineWave.push(x, y + canvas.height * CHART_HEIGHT_RATIO);
    });
