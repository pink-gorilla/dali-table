(ns notebook.dali.rtable.chart.vega
  (:require
   [rtable.plot.vega :refer [vega]]))

(vega {:embed/opts {; see:https://vega.github.io/vega-embed/interfaces/EmbedOptions.html
                    :width 400
...
(vega {:embed/opts {:width 400 :height 247.2187886279357
                    :padding {:top 10, :left 55, :bottom 40, :right 10}}
       :spec {:$schema "https://vega.github.io/schema/vega/v5.json",
              :data [{:name "6348d171-8e7b-4a02-8b9a-bea64dbcbdce",
                      :values [{:x 0, :y 7} {:x 1, :y 8} {:x 2, :y 7} {:x 3, :y 4} {:x 4, :y 6}]}],
              :marks [{:type "symbol", :from {:data "6348d171-8e7b-4a02-8b9a-bea64dbcbdce"},
                       :encode {:enter {:x {:scale "x", :field "x"},
                                        :y {:scale "y", :field "y"}, :fill {:value "steelblue"}, :fillOpacity {:value 1}},
                                :update {:shape "circle", :size {:value 70}, :stroke {:value "transparent"}},
                                :hover {:size {:value 210}, :stroke {:value "white"}}}}],
              :scales [{:name "x", :type "linear", :range "width", :zero false, :domain {:data "6348d171-8e7b-4a02-8b9a-bea64dbcbdce", :field "x"}}
                       {:name "y", :type "linear", :range "height", :nice true, :zero false, :domain {:data "6348d171-8e7b-4a02-8b9a-bea64dbcbdce", :field "y"}}],
              :axes [{:orient "bottom", :scale "x"} {:orient "left", :scale "y"}]}})

(vega {:embed/opts {:width 400 :height 247.2187886279357
                    :padding {:top 10, :left 55, :bottom 40, :right 10}}
       :spec {:$schema "https://vega.github.io/schema/vega/v5.json",
              :data [{:name "9612b327-d447-478a-84a6-27af98f319f0", 
                      :values [{:x 0, :y 7} {:x 1, :y 8} {:x 2, :y 7} {:x 3, :y 4} {:x 4, :y 6}]}],
              :marks [{:type "symbol", :from {:data "9612b327-d447-478a-84a6-27af98f319f0"},
                       :encode {:enter {:x {:scale "x", :field "x"}, :y {:scale "y", :field "y"}, :fill {:value "steelblue"}, :fillOpacity {:value 1}},
                                :update {:shape "circle", :size {:value 70}, :stroke {:value "transparent"}}, :hover {:size {:value 210}, :stroke {:value "white"}}}}],
              :scales [{:name "x", :type "linear", :range "width", :zero false, :domain {:data "9612b327-d447-478a-84a6-27af98f319f0", :field "x"}}
                       {:name "y", :type "linear", :range "height", :nice true, :zero false, :domain {:data "9612b327-d447-478a-84a6-27af98f319f0", :field "y"}}],
              :axes [{:orient "bottom", :scale "x"} {:orient "left", :scale "y"}]}})


;; rect plot
(vega {:embed/opts {:width 400 :height 400
...
;; zoom 
(vega {:embed/opts {:width 500 :height 400
       ...
       :axes [{:scale "xscale", :orient "top", :offset {:signal "xoffset"}}
       ...
       :signals [{:name "margin", :value 20}
                 {:name "hover" :on [{:events "*:mouseover", :encode "hover"}
                 ...
                 {:name "down" :value nil :on [{:events "touchend", :update "null"}
                                               {:events "mousedown, touchstart", :update "xy()"}]}
                 {:name "xcur" :value nil :on [{:events "mousedown, touchstart, touchend"
                                                :update "slice(xdom)"}]}
                 {:name "ycur" :value nil :on [{:events "mousedown, touchstart, touchend"
                                                :update "slice(ydom)"}]}
                 {:name "delta" :value [0 0] :on [{:events
                                                   [{:source "window"
                                                     :type "mousemove"
                                                     :consume true
                                                     :between
